...
contract TestBearcoin is ERC20, Ownable, KeeperCompatibleInterface, VRFConsumerBase {
  ...
  uint256 private _testBitcoinPrice = _genesisBitcoinPrice;
  ...
  //In test mode, we use a separate variable to track the current bitcoin price
  //because a) we can't fetch it and b) it needs to be separate from the current price to track % changes
  function devSetCurrentBitcoinPrice( uint256 price ) public onlyOwner {
    _testBitcoinPrice = price;
  ...
  //In test mode, does nothing and just returns the current price
  function _fetchBitcoinPrice() internal view returns (int256) {
    return int256(_testBitcoinPrice);
  }
  ...
  //Have to comment out random execution of each action (for consistent results), as well as requestRandomness
